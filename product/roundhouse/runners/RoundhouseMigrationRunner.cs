...
namespace roundhouse.runners
{
    public sealed class RoundhouseMigrationRunner : IRunner
    {
        ...
        public void run()
        {
            ...
            Log.bound_to(this)
                .log_an_info_event_containing("Looking in {0} for scripts to run.", known_folders.up.folder_path);
            if (!silent)
            {
            ...
            Log.bound_to(this)
                .log_a_debug_event_containing("The change_drop (output) folder is: {0}",
                    known_folders.change_drop.folder_full_path);
            Log.bound_to(this)
                .log_a_debug_event_containing("Using SearchAllSubdirectoriesInsteadOfTraverse execution: {0}",
                    configuration.SearchAllSubdirectoriesInsteadOfTraverse);
            ...
            try
            {
                ...
                if (!dropping_the_database)
                {
                    ...
                    Log.bound_to(this)
                        .log_an_info_event_containing(" Migrating {0} from version {1} to {2}.",
                            database_migrator.database.database_name, current_version, new_version);
                    ...
                    {
                        log_and_traverse(known_folders.run_after_create_database, version_id, new_version,
                    ...
                    log_and_traverse(known_folders.run_after_other_any_time_scripts, version_id, new_version,
                        ConnectionType.Default);
        ...
        public void log_and_traverse(MigrationsFolder folder, long version_id, string new_version,
            ConnectionType connection_type)
        {
            ...
            Log.bound_to(this).log_an_info_event_containing("{0}", "-".PadRight(50, '-'));
            traverse_files_and_run_sql(folder.folder_full_path, version_id, folder, environment, new_version,
                connection_type);
        }
            ...
            string repository_version, ConnectionType connection_type)
        {
            ...
            {
                var sql_file_text = replace_tokens(get_file_text(sql_file));
                Log.bound_to(this).log_a_debug_event_containing(" Found and running {0}.", sql_file);
                var the_sql_ran = database_migrator.run_sql(sql_file_text, file_system.get_file_name_from(sql_file),
                ...
                if (the_sql_ran)
                {
                    ...
                    {
                        Log.bound_to(this)
                            .log_a_warning_event_containing("Unable to copy {0} to {1}. {2}{3}", sql_file,
            ...
            foreach (string child_directory in file_system.get_all_directory_name_strings_in(directory))
            {
                traverse_files_and_run_sql(child_directory, version_id, migration_folder, migrating_environment,
                    repository_version, connection_type);
            }
        }
        ...
        private void copy_to_change_drop_folder(string sql_file_ran, Folder migration_folder)
        {
            ...
            {
                string destination_file = file_system.combine_paths(known_folders.change_drop.folder_full_path,
                    "itemsRan",
                ...
                Log.bound_to(this)
                    .log_a_debug_event_containing("Copying file {0} to {1}.",
                        file_system.get_file_name_from(sql_file_ran), destination_file);
