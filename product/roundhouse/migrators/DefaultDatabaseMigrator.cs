...
namespace roundhouse.migrators
{
    public sealed class DefaultDatabaseMigrator : DatabaseMigrator
    {
        ...
        public DefaultDatabaseMigrator(Database database, CryptographicService crypto_provider,
        ...
        public bool create_or_restore_database(string custom_create_database_script)
        {
            ...
            {
                Log.bound_to(this)
                    .log_an_info_event_containing("Creating {0} database on {1} server if it doesn't exist.",
                        database.database_name, database.server_name);
            }
            else
            {
                Log.bound_to(this)
                    .log_an_info_event_containing("Creating {0} database on {1} server with custom script.",
                        database.database_name, database.server_name);
            }
        ...
        {
            Log.bound_to(this)
                .log_an_info_event_containing("Restoring {0} database on {1} server from path {2}.",
        ...
        public void set_recovery_mode(bool simple)
        {
            ...
            Log.bound_to(this)
                .log_an_info_event_containing("Setting recovery mode to '{0}' for database {1}.",
                    simple ? "Simple" : "Full", database.database_name);
        ...
        public void run_roundhouse_support_tasks()
        {
            ...
            Log.bound_to(this)
                .log_an_info_event_containing(" Creating [{0}] table if it doesn't exist.", database.version_table_name);
            Log.bound_to(this)
                .log_an_info_event_containing(" Creating [{0}] table if it doesn't exist.",
        ...
        {
            Log.bound_to(this)
                .log_an_info_event_containing("Deleting {0} database on {1} server if it exists.",
                    database.database_name, database.server_name);
        ...
        public bool run_sql(string sql_to_run, string script_name, bool run_this_script_once,
            bool run_this_script_every_time, long version_id, Environment environment, string repository_version,
            string repository_path, ConnectionType connection_type)
        {
            var this_sql_ran = false;

            if (this_is_a_one_time_script_that_has_changes_but_has_already_been_run(script_name, sql_to_run,
                ...
                if (error_on_one_time_script_changes)
                {
                    ...
                    string error_message =
                        string.Format(
                            ...
                            script_name);
                    record_script_in_scripts_run_errors_table(script_name, sql_to_run, sql_to_run, error_message,
                        repository_version, repository_path);
                    database.close_connection();
                    throw new Exception(error_message);
                }
                Log.bound_to(this)
                    .log_a_warning_event_containing("{0} is a one time script that has changed since it was run.",
                        script_name);
            }
            ...
            {
                Log.bound_to(this)
                    .log_an_info_event_containing(" Running {0} on {1} - {2}.", script_name, database.server_name,
                ...
                foreach (var sql_statement in get_statements_to_run(sql_to_run))
                {
                    ...
                    {
                        Log.bound_to(this)
                            .log_an_error_event_containing("Error executing file '{0}': statement running was '{1}'",
                                script_name, sql_statement);
                        database.rollback();
            ...
            {
                Log.bound_to(this)
                    .log_an_info_event_containing(" Skipped {0} - {1}.", script_name,
            ...
            if (database.split_batch_statements)
            {
                foreach (
                    var sql_statement in
                        StatementSplitter.split_sql_on_regex_and_remove_empty_statements(sql_to_run,
                            database.sql_statement_separator_regex_pattern))
                {
        ...
        {
            Log.bound_to(this)
                .log_a_debug_event_containing("Recording {0} script ran on {1} - {2}.", script_name,
                    database.server_name, database.database_name);
            database.insert_script_run(script_name, sql_to_run, create_hash(sql_to_run), run_this_script_once,
                version_id);
        }

        public void record_script_in_scripts_run_errors_table(string script_name, string sql_to_run,
            string sql_erroneous_part, string error_message, string repository_version, string repository_path)
        {
            Log.bound_to(this)
                .log_a_debug_event_containing("Recording {0} script ran with error on {1} - {2}.", script_name,
                    database.server_name, database.database_name);
            database.insert_script_run_error(script_name, sql_to_run, sql_erroneous_part, error_message,
                repository_version, repository_path);
        }
        ...
        private bool this_is_a_one_time_script_that_has_changes_but_has_already_been_run(string script_name,
            string sql_to_run, bool run_this_script_once)
        {
            return this_script_has_changed_since_last_run(script_name, sql_to_run) &&
                   this_script_has_run_already(script_name) && run_this_script_once;
        }
        ...
        private bool this_script_has_changed_since_last_run(string script_name, string sql_to_run)
        {
            ...
            {
                Log.bound_to(this)
                    .log_a_warning_event_containing("{0} - I didn't find this script executed before.", script_name);
            }
                ...
                if (hash_is_same)
                {
                    Log.bound_to(this)
                        .log_a_warning_event_containing(
                            "Script {0} had different line endings than before but equal content", script_name);
                }
            }
        ...
        private bool this_script_should_run(string script_name, string sql_to_run, bool run_this_script_once,
            bool run_this_script_every_time)
        {
        ...
        public bool this_is_an_environment_file_and_its_in_the_right_environment(string script_name,
            Environment environment)
        {
            Log.bound_to(this)
                .log_a_debug_event_containing(
                    "Checking to see if {0} is an environment file. We are in the {1} environment.", script_name,
                    environment.name);
            ...
            Log.bound_to(this)
                .log_an_info_event_containing(
                    " {0} is an environment file. We are in the {1} environment. This will{2} run based on this check.",
