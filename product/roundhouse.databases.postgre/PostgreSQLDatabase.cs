...
namespace roundhouse.databases.postgresql
{
	...
	public class PostgreSQLDatabase : AdoNetDatabase
	{
		...
		public override void initialize_connections(ConfigurationPropertyHolder configuration_property_holder)
		{
			...
			{
				var builder = new Npgsql.NpgsqlConnection(connection_string);
		...
		public override void create_or_update_roundhouse_tables()
		{
			...
			run_sql(TableCreationScripts.create_roundhouse_scripts_run_table(roundhouse_schema_name, version_table_name, scripts_run_table_name), ConnectionType.Default);
			Log.bound_to(this).log_an_info_event_containing("Creating table [{0}].", scripts_run_errors_table_name);
			run_sql(TableCreationScripts.create_roundhouse_scripts_run_errors_table(roundhouse_schema_name, scripts_run_errors_table_name), ConnectionType.Default);
		}
